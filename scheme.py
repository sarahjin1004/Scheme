...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        operands = operands.map(lambda x: scheme_eval(x, env))
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    bindings.map(lambda p: check_form(p, 2, 2))
    symbols = bindings.map(lambda p: p.first)
    pre_values = (bindings.map(lambda p: p.second))
    check_formals(symbols)
    values = pre_values.map(lambda p: scheme_eval(p.first, env))
    let_frame = env.make_child_frame(symbols, values)
    return let_frame
    # END PROBLEM 15
